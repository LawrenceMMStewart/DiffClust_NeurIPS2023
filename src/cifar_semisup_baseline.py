...
from models import BaselineBNTrainState, Baseline
...
"""
Classification (semi-supervised setting) CIFAR 10
using a RESNET 50 architecture (modified for CIFAR).
Used as a baseline to compare clustering performance
and downstream classification (transfer learning).
"""
...
def train_step_fn(state, X, Yhot, rngs):
    def forward(params, batch_stats, X, Yhot, rngs):
        (loss, outs), mutables = state.apply_fn({'params' : params, 'batch_stats' : batch_stats},
                                                   X, Yhot, rngs=rngs, training=True, mutable=['batch_stats'])
        return loss, (outs, mutables)
    (loss, (outs, mutables)), grads = jax.value_and_grad(forward, has_aux=True)(state.params, state.batch_stats, X, Yhot, rngs)
...
def eval_step_fn(state, X, Yhot, rngs):
    ce, outs = state.apply_fn({'params' : state.params,
                               'batch_stats' : state.batch_stats}, X, Yhot, training=False, rngs=rngs)
    return outs
...
def create_state(rngs, backbone=ResNet50(), optimizer=optax.sgd(1e-3), dshape=(32, 32, 3), bs=64):
    model = Baseline(backbone=backbone, num_classes=10)

    dummy_x = jnp.ones((bs,) + dshape)
    dummy_yhot = jnp.ones((bs, 10))
    variables = model.init(rngs, dummy_x, dummy_yhot, training=False)

    print(parameter_overview.get_parameter_overview(variables['params']))
    state  = BaselineBNTrainState.create(apply_fn=model.apply,
...
def train(config : ml_collections.ConfigDict, writer : metric_writers.SummaryWriter, handler):
    ...
    for step in range(1, config.steps + 1):
        ...
        X, Yhot = handler.next_labeled(augment=True, key=rngs['augmentations'])
    ...
    if config.save:
        ...
        for h in config.save_hooks:
            ...
            def test_single_batch(state, x, yhot, rngs):
                return state.clust_eval_fn({'params' : state.params, 'batch_stats' : state.batch_stats}, x, yhot, training=False, rngs=rngs)
    ...
    if config.save:
        ...
        for h in config.save_hooks:
            ...
            for dset in ['downstream', 'val', 'test']:
                ...
                V = state.embed_fn({'params' : state.params, 'batch_stats' : state.batch_stats}, X, training=False, rngs=rngs)
...
def visualize_embeddings(config : ml_collections.ConfigDict, handler):
    ...
    if config.save:
        for hook in config.save_hooks:
            ...
            V_val = state.embed_fn({'params' : state.params, 'batch_stats' : state.batch_stats}, X_val, training=False, rngs=rngs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
                    prog='Semisupervised CIFAR10 Baseline',
                    description='Train RESNET50 in semi-supervised setting (baseline)')
