...
from models import DCVanillaSemiSup, get_lpfuncs, DCTrainState
from backbones import CNN, MLP
import utils
from flax.training import orbax_utils
import optax, orbax
from nistdata import NISTClustSemiSupervised
...
def train_step_fn(state, X, Yhot, batch_mask, ncc, rngs):
    def forward(params, X, Yhot, batch_mask, ncc, rngs):
        return state.forward_fn({'params' : params}, X, Yhot, batch_mask, ncc, rngs['noise'], True, True, rngs=rngs)
    (loss, outs), grads = jax.value_and_grad(forward, has_aux=True)(state.params, X, Yhot, batch_mask, ncc, rngs)
...
def create_state(rngs, backbone=CNN(), optimizer=optax.sgd(1e-3), dshape=(28, 28, 1), sigma=0.1, num_samples=100,
    ...
    #tmp
    bbparams = model.backbone.init(rngs, dummy_x, training=True)['params']
    params = params.unfreeze()
    params['backbone'] = bbparams
...
def train(config : ml_collections.ConfigDict, writer : metric_writers.SummaryWriter, handler):
    ...
    for step in range(1, config.steps + 1):
        ...
        batch_mask, (X, Yhot) = handler.next_train()
